...
export function ProjectDetail({ project, onBack }) {
  ...
  const fetchUserTeams = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/teams/hackathon/${project.hackathon._id}`, {
        headers: {
  ...
  const fetchProjectTeams = async () => {
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(`http://localhost:3000/api/teams/project/${project._id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
...
useEffect(() => {
  ...
  const handleUpdateDescription = async () => {
    ...
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(`http://localhost:3000/api/teams/${editingTeamId}/description`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const confirmRemoveMember = async () => {
    ...
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(`http://localhost:3000/api/teams/${removeDialog.teamId}/members/${removeDialog.memberId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const confirmLeaveTeam = async () => {
    ...
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(`http://localhost:3000/api/teams/${leaveDialog.teamId}/leave`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
          ...
          <p className="font-semibold text-gray-900">
            {project.submittedBy.name || "Unknown"}
                    ...
                    {projectTeams.length === 0 ? (
                      <div className="text-center py-8">
                        ...
                        <p className="text-gray-500 mb-4">
                          Create a team to collaborate on this project and invite members to join.
                        </p>
                        <Button
                          onClick={() => setShowCreateTeamModal(true)}
                          className="flex items-center gap-2"
                ...
                {(project.team || []).length > 0 && (
                  <Card>
                    <CardHeader><CardTitle><Users className="w-5 h-5" /> Project Team Members</CardTitle></CardHeader>
                    <CardContent>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md relative">
            ...
            <div className="mb-4 p-2 bg-blue-50 border border-blue-200 rounded text-xs text-blue-800">
              ...
              This hackathon allows teams of {project.hackathon?.teamSize?.min || 1} to {project.hackathon?.teamSize?.max || 4} members.
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <Button 
                  ...
                  disabled={!newTeamName.trim() || !newTeamDescription.trim() || loading}
                  className="flex-1"
                >
                  {loading ? (
                    <>
