...
export function AnnouncementsPanel() {
  ...
  const getAudienceColor = (aud) => {
    switch (aud) {
      case "all": return "bg-purple-500 text-white border-purple-500/30"
      case "organizers": return "bg-blue-500 text-white border-blue-500/30"
      case "participants": return "bg-green-500 text-white border-green-500/30"
      case "mentors": return "bg-yellow-500 text-white border-yellow-500/30"
      default: return "bg-gray-500 text-white border-gray-500/30"
    }
  }
  ...
  <div className="w-full lg:w-1/2 flex-shrink-0">
    <Card>
      ...
      <CardContent className="space-y-4">
        <div>
          <label className="text-gray-700 text-sm font-medium mb-2 block">Title</label>
          <Input
        ...
        <div>
          <label className="text-gray-700 text-sm font-medium mb-2 block">Audience</label>
        ...
        <div className="flex space-x-3 pt-2">
          ...
          <Button
            ...
            className="flex-1 bg-gradient-to-r from-purple-500 to-blue-500 hover:bg-white hover:text-black"
          >
          ...
          {announcements.map((announcement) => (
            <div key={announcement._id} className="p-4 bg-white/5 rounded-lg border border-purple-500/20">
              <div className="flex items-start justify-between mb-2">
                <h4 className="text-black font-medium text-sm">{announcement.title}</h4>
                <Badge className={getAudienceColor(announcement.audience)}>{announcement.audience}</Badge>
              </div>
              <p className="text-gray-600 text-xs mb-3 line-clamp-2">{announcement.message}</p>
              <div className="flex items-center justify-between">
                <span className="text-gray-700 text-xs">{new Date(announcement.createdAt).toLocaleString()}</span>
                <Badge variant="secondary" className="bg-green-500 text-green-600 border-green-500/30">
                  Sent
                </Badge>
              </div>
            </div>
