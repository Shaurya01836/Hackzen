// Test script to check submission status after shortlisting
...
async function testSubmissionStatus() {
  try {
    console.log('🧪 Testing submission status...');
    ...
    console.log('✅ Submissions data:', {
      ...
      pendingSubmissions: response.data.filter(s => s.status === 'pending').length,
...
// Test leaderboard to see what status is returned
...
// Main test function
async function testSubmissionStatus() {
  console.log('🚀 Starting submission status tests...\n');
  
  // Test 1: Get hackathon round progress
  const hackathonData = await testHackathonRoundProgress();
  
  // Test 2: Get submissions status
  const submissions = await testSubmissionStatus();
  
  // Test 3: Get leaderboard status
...
if (require.main === module) {
  testSubmissionStatus().catch(console.error);
}
